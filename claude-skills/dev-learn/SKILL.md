---
name: dev-learn
description: "Use after solving a significant problem, before shipping work with knowledge artifacts, after shipping a feature, after a sprint, or when reusable workflow knowledge should be captured."
---
...
|---|---|
| 解决了问题/定位了根因 | Route A |
| sprint/weekly retro | Route B |
| 发现可复用方法 | Route C |
| 知识库过期 | Route D |
| skill gap 反复出现 | Route E |
| 外部资料入库 | Route F |

## Common Mistakes

- pre-ship 调用后直接跳到下一项工作：必须先回到 Phase 5.5 / 6
- 在没有验证 discoverability 的情况下写完就结束
- 把 skill 改进写成直接 patch：Route E 要遵守 `writing-skills`
- 用不存在的本地校验资产阻塞沉淀流程
...
Verified or completed work
  |
  +-- [Problem solved] -----------> Route A: Compound protocol
  |   "fixed", "root cause",        -> docs/solutions/<category>/<slug>.md
  |   "the issue was"
  |
  +-- [Sprint/week end] ----------> Route B: gstack-retro (OPTIONAL)
  ...
  |   "encode as a skill"
  |
  +-- [Knowledge stale] ----------> Route D: Compound refresh protocol
...
0. **Skill self-review (L2 信号收集)**
   ...
   - 脚本路径解析：`REPO_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"`；若 `$REPO_ROOT/scripts/supervise/extract_signals.py` 不存在 → 宣告"supervise 脚本未就绪，跳过本步"并继续
...
2. **Execute detected route**
   ...
   **Route A: Execute the compound protocol from `references/compound-engine.md`** (problem documentation)
   - 默认策略：**Full + session history**，直接执行，不先问 "Full 还是 Lightweight"。
   - 只有以下情况才改默认：
     - 用户明确说"轻量"/"简单记一下" -> Lightweight
     - 用户明确说"不要查会话历史" -> Full without session history
     - 明确 blocker：拿不到 session 历史 -> 报告并询问
   ...
   **Route B: `gstack-retro`** (OPTIONAL retrospective enhancement)
   - gstack-retro 未安装 -- 使用 git log 手动分析提交频率和模式。
   - 若已安装：analyzes commit frequency, code LOC, test ratios, file hotspots, AI-assisted %
   ...
   **Route D: Execute the compound protocol from `references/compound-engine.md`** (refresh mode)
   ...
   **Route E: `writing-skills`** (skill improvement, REFACTOR mode)
   - RED: run a baseline pressure scenario against the old skill / old routing and document the exact failure or false positive
   - Analyze gap: which route/rule/signal is missing or outdated
   - GREEN: make the minimal `SKILL.md` / workflow change that addresses that recorded failure
   - REFACTOR: re-run the same scenario, collect new rationalizations, and close loopholes before declaring the skill improved
   - Keep `SKILL.md` + `docs/ai-coding-workflow.md` in sync

   **Route F: Wiki Ingest** (knowledge compilation)
   - 委托给 **`/dev:wiki-ingest`** —— 它负责抽取实体/概念/决策、规划页面改动、写入 wiki

3. **Wiki Ingest (auto, after Route A and B)**
   - After compound protocol writes `docs/solutions/*.md` -> 自动调 `/dev:wiki-ingest` 处理项目 wiki
   - After retro completes -> 自动调 `/dev:wiki-ingest` 处理 retro 发现
   - Announce (中文): "Solution 已写入 docs/solutions/。委托 dev:wiki-ingest 编译进 wiki..."

4. **Verify knowledge is discoverable**
...
5. **Return to caller context**
   - **Pre-ship Phase 5.5 invocation**: return to `dev-flow` Phase 5.5. If Route A/B/F wrote files, refresh Phase 5 PASS evidence for the current HEAD/tree/diff before Phase 6.
   - **Post-ship Phase 7 invocation**: proceed to `/dev:discover` (Phase 1) for the next work item.
   - **Standalone user invocation**: report artifacts and ask only if there is a real next-work decision.

## Knowledge Feedback Loop

```
Phase 4 (code) -- solve problem --> Phase 7 (compound protocol) -- docs/solutions/
                                                                      |
Phase 3 (plan) <-- learnings-researcher auto-searches ----------------+
Phase 5 (review) <-- learnings-researcher always-on -----------------+
...
| **Next** | Pre-ship: return to `dev-flow` Phase 5.5 -> Phase 6 after refreshed verification. Post-ship: `/dev:discover` (Phase 1). |
