---
name: dev-flow
description: "Use when starting development work, resuming an interrupted task, deciding which dev:* phase applies, handling bug reports, or preparing verified work for delivery."
...
2. **工作区前置（强制）。** 在创建任何文档或代码之前，必须确认当前位于任务专属 git worktree：`<repo>/.worktrees/<task-name>/`。若在 main/master、普通主 checkout feature branch、或路径不在 `.worktrees/` 下，STOP 并调用 `using-git-worktrees` 创建工作区后再继续。用户拒绝创建时，STOP 并要求显式确认"在当前分支继续"；main/master 永不允许写入 override。
...
- 不确定当前该从哪个 `dev:*` phase 开始
- 跨会话恢复中断工作
- 用户给的是 bug、想法、plan path、requirements path、或交付意图，需要自动判定入口
- 想减少普通阶段的确认，只在 blocker/重大取舍时停下

**Skip when:** 用户明确要求直接运行某个单独 phase，且当前上下文没有路由歧义。

## Quick Reference

| Situation | Enter At |
|---|---|
| 新需求/模糊想法 | Phase 1 `dev-discover` |
| 已有 requirements，涉及 UI | Phase 2 `dev-design` |
| 已有 requirements，无 plan | Phase 3 `dev-plan` |
| 已有 plan，开始实现 | Phase 4 `dev-code` |
| 代码完成待审查 | Phase 5 `dev-verify` |
| 审查通过待沉淀/交付 | Phase 5.5 -> Phase 6 |
| 已交付后的复盘/沉淀 | Phase 7 `dev-learn` |

## Common Mistakes

- 把 `mode:auto` 当成 ship 授权：它只自动推进非破坏性阶段
- 在普通 feature branch 继续写文件：写入型 phase 必须在 `<repo>/.worktrees/<name>/`
- 把旧 review 记录当成当前 diff 的 PASS 证据：证据必须绑定当前 HEAD/tree/diff
- 用户只说“ship it”就默认写 main：Path A 需要明确 main 授权
...
| `mode:auto` | Pass `mode:auto`, or explicitly ask for "全自动模式" / "自动跑完整个流程" / "不要每步确认" | Same as guided for non-destructive phase checkpoints; does not authorize commit/push/PR/merge/deploy |

Mode rules:
- Default guided mode and `mode:auto` both auto-advance non-destructive phase checkpoints when "当前阶段通过 + 下一步唯一明确"。
- `mode:auto` is **not** ship authorization. Phase 6 still requires a separate `/dev:ship` invocation or explicit "提交/commit/push/PR/合并到 main/上线/deploy" wording.
...
- 进入 Phase 6 但没有显式 ship 授权；`mode:auto` 不算 ship 授权
...
| On a branch but repo top-level is not `<repo>/.worktrees/<task-name>/` | STOP: "当前不是任务专属 worktree。调用 `using-git-worktrees` 创建独立工作区,路径 `<repo>/.worktrees/<task-name>/`。" |
| User refuses to create a worktree | STOP: "需要用户显式确认 `在当前分支继续` 才能跳过工作区前置。" |
| Already in `<repo>/.worktrees/<task-name>/` | 继续 Signal 1 |
...
| Uncommitted code changes + Phase 5 PASS evidence bound to current HEAD/tree/diff | Phase 5.5 (pre-ship knowledge/supervision) -- decide whether to run `dev-learn` / `dev-supervise`, then Phase 6 |
...
| "ship it" / "deploy" / "create PR" / "提交" / "push" / "合并到 main" | Phase 5.5 (pre-ship knowledge/supervision) -- then Phase 6 delivery |
...
# Current-diff Phase 5 PASS evidence
find .context/compound-engineering/ce-review -maxdepth 2 -type f 2>/dev/null
```
...
| Code changes + current-diff Phase 5 PASS evidence | Phase 5.5 (pre-ship knowledge/supervision) -> Phase 6 |
...
[Entry Point Detected]
  ...
  Phase 1: /dev:discover
      |  Scene Detection -> ce-ideate / office-hours / ce-brainstorm
  ...
  Phase 5: /dev:verify
      ...
      |  GATE: PASS verdict bound to current HEAD/tree/diff
      |
      v
  Phase 5.5: Pre-Ship Knowledge/Supervision Decision
      ...
      |  GATE: no required pre-ship capture remains; if files changed, Phase 5 evidence refreshed
      |
      v
  Phase 6: /dev:ship
      |  Requires separate ship authorization
      |  Select Path A only when user explicitly asked to merge/push main
      |  gstack-land-and-deploy with canary
...
- Phase 6 commit/push/PR 尚未获得显式 ship 授权；`mode:auto` 不算授权
...
| 本次 diff 修改 `claude-skills/`、`.agents/skills/`、`docs/ai-coding-workflow.md`、`scripts/supervise/` 或 `docs/supervise/` | 先运行 `dev-supervise --scope project --since 7d`；若暴露 skill gap，开启 Phase 4/5 子流程并按 `writing-skills` RED-GREEN-REFACTOR 修复，不在 Phase 5.5 内直接 patch |
...
- 任一步骤写入新文件后，必须至少执行针对性验证（例如 `git status --short`、相关 markdown grep/链接检查、必要时 `/dev:verify` 的 docs-only 审查），并刷新绑定当前 HEAD/tree/diff 的 Phase 5 PASS 证据，再进入 Phase 6。
- `dev-supervise` 只产出报告，不自动 patch `SKILL.md`；如报告要求修改技能，回到 Phase 4/5 完成修改和 `writing-skills` 验证后再重新进入 Phase 5.5。
...
Phase 1 (/dev:discover):
  ...
  -> gstack-design-shotgun 生成 3 个变体
  -> 用户选择变体 B
...
Phase 6 (/dev:ship):
  场景: 用户明确要求合并到 main，且无版本文件 -> Path A (squash 合并回 main)
  -> 主干同步: rebase origin/main，无冲突
  -> squash merge: 展示 diff 摘要 + 拟定 commit message
  -> 已有 main 授权且无 blocker: 自动继续
