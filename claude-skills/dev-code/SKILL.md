...
4. **Review 自动修复回路。** `dev-code` 是外层编排器：按 2-3 个 Implementation Units 切批执行 work protocol，每批后运行 autofix review protocol；所有 Unit 完成后先执行 simplify protocol，再运行最终 autofix。Phase 4 只自动应用 `safe_auto`，不做 `gated_auto/manual` 用户裁决；残余发现进入 Phase 5 的总体审查。

## Overview

Phase 4 executes the plan. The work engine **detects** plan complexity and **auto-selects** the best execution strategy. Auxiliary skills are **auto-triggered** by context signals -- not by the user.
...
- No plan file, has bare prompt -> work engine assesses complexity:
...
| Measurable optimization goal (prompt quality, latency, search relevance) | Execute optimization protocol from `references/optimize-engine.md` |
...
| Goal is measurable metric improvement | Execute optimization protocol from `references/optimize-engine.md` |
...
2. **`dev-code` slices execution into batches, then executes the work protocol from `references/work-engine.md` for each batch**
   ...
   - Pass the plan path plus the current batch's Unit IDs/goals; execute only that batch.
   - Use work engine as an implementation executor only: Implement -> Test Discovery -> System-Wide Test Check -> stage allowed, **no commit**.
   - Do not enter shipping workflow / residual-review gate. Phase 4 residuals are recorded and handed to Phase 5.

3. **REVIEW AUTOFIX LOOP: Execute the autofix review protocol from `references/review-autofix.md`** (Phase 4 自动修复回路)
   - 默认 Tier 2: 20+ persona 并行审查, `safe_auto` 修复, R-ID 追溯
   - Tier 1 (仅当全部满足: 纯新增 + 单一关注点 + 模式跟随 + 忠于计划)
   - 传入 `plan:<path>` 用于需求追溯验证
   - **批次触发**: 每个 work batch 完成后立即运行一次；若是单批次/小改动，至少在实现完成后运行一次
   - **最终触发**: 所有 Unit 完成并运行 simplify protocol 后，再运行一次最终 autofix，覆盖 simplification edits
   - **单次运行内部循环**: autofix 自动执行 bounded re-review，应用 `safe_auto -> review-fixer` 后重审，直到无新的 `safe_auto` 或达到轮次上限
...
4. **Phase 4 → Phase 5 过渡关：simplify protocol + final autofix**
   - 所有 Unit 完成后、进入 `/dev:verify` 之前，执行 simplify protocol from `references/simplify-protocol.md`
...
| **Output** | 已修改的代码 + 通过的测试 + Phase 4 多次 autofix review 的 `safe_auto` 已应用 + simplify protocol 已扫过（**未 commit**，工作树留有变更） |
