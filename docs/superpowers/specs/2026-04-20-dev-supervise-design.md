...
| **L2 会话复盘** | `dev-learn` 内置子步 `skill-self-review` | Phase 5.5 pre-ship 或 Phase 7 | 本 session 信号 → `docs/supervise/feedback/<skill>.md` |
| **L3 历史扫描** | `/dev:supervise` 手动命令 | 用户触发 | 跨 session 聚合排行 + 结构化改进建议 |

`dev-supervise` 不作为常规业务 phase 进入 `dev-flow` 串行链；但 `dev-flow` 的 Phase 5.5 交付前自省判断可在出现 skill/process 风险信号时按需触发 L3 project-scope 扫描。其他入口为 L3 命令与 L2 子步。
...
- 不作为 `dev-flow` 的常规业务 phase 自动编排；只允许 Phase 5.5 在有自省信号时按需触发
- 入口仅:`/dev:supervise`(L3)、`dev-learn` 子步(L2)、`dev-flow` Phase 5.5 按需触发的 project-scope 自省扫描
