...
| 检测到的信号 | 自动触发技能 |
|---|---|
| Implementation Unit 标记 `test-first` | `tdd`（垂直切片 RED-GREEN-REFACTOR：一个测试 → 一个实现 → 重复；禁止水平切片"先写所有测试再写所有实现"；测试只验证公共接口行为，不验证实现细节；接口设计追求 deep module；mock 规则见 `tdd/mocking.md`） |
