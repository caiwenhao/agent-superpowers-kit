...
| 信号 | 检测方式 | 路由结果 |
|---|---|---|
| Signal 0: 是否在任务专属工作区 | `git rev-parse --abbrev-ref HEAD` + `git rev-parse --show-toplevel` + `git worktree list` | 在 main/master、普通主 checkout feature branch、或未位于 `<repo>/.worktrees/<task-name>/` -> 调用 `using-git-worktrees` 先创建工作区，再进入任何写入型 Phase |
...
`mode:auto` 表示用户授权自动推进非破坏性、非交付步骤；它**不**授权 `git commit` / `git push` / PR / merge / deploy。进入 Phase 6 仍必须有单独 ship 授权：用户显式调用 `/dev:ship`，或明确说"提交/commit/push/PR/合并到 main/上线/deploy"。
...
| 依赖升级 | Phase 2；仍需 Phase 1 lightweight requirement 或等价 Issue/spec 后进入 Phase 3 |
...
┌─────────────────────────────────────────────┐
│  Step 1: 复现                                │
│  reproduce-bug + diagnose Phase 1            │
│  (涉及 UI 时叠加 dev-browser)                │
│  diagnose 的核心：构建 feedback loop          │
│  （失败测试 > curl > CLI > headless browser  │
│    > replay trace > throwaway harness        │
│    > property fuzz > bisection > HITL）      │
│  产出：可稳定触发的复现步骤 + 失败断言        │
│  ⛔ 无法复现 -> 停下，向用户要更多上下文       │
└─────────────────────────────────────────────┘
    |
...
┌─────────────────────────────────────────────┐
│  Step 2: 根因定位                            │
│  diagnose Phase 3-4（假设 -> 仪器化）        │
│  - 生成 3-5 个可证伪假设，排序后逐一验证     │
...
| 用户粘贴 stack trace / 错误日志 | 日志作为假设输入；可先做只读定位，但进入 Step 2 有效根因定位/任何修复前仍必须产出可验证失败断言、最小复现、或 characterization test |
...
- 如果根因分析暴露架构摩擦（无好的测试 seam、耦合过深、隐藏依赖），修复完成后推荐 `improve-codebase-architecture` 做深化分析——**修复之后提，不是修复之前**。

#### 铁律（Bug Fix 专属补充）

- **无复现，不定位** —— 不能稳定复现或固化失败断言的 bug 不进入 Step 2；stack trace 不是复现，只是线索
...
| `grill-with-docs` | 领域对齐 grilling | brainstorm 内部的 grilling 引擎：挑战计划 vs 现有领域模型，锐化术语，即时更新 CONTEXT.md + ADR |
| `grill-me` | 通用决策穷举 | 非代码决策（产品方向、架构取舍、流程设计）需要穷举决策树时 |
| `to-prd` | 需求文档快速合成 | 对话上下文已充分（grilling 完成后），直接合成 PRD 而不再访谈；适合已有 Issue Tracker 的项目 |
| `document-review` | 多人格审查 | Route C/Deep 或高风险需求强制；Route D 小范围低风险 opt-in |

### 领域词汇表（CONTEXT.md）

brainstorm / grilling 过程中，如果项目根目录无 `CONTEXT.md`，在第一个领域术语被澄清时**懒创建**。词汇表让 agent 用 1 个词替代 20 个词的解释，命名一致、token 更省。格式遵循 `grill-with-docs` 的 CONTEXT-FORMAT.md。

已有 `CONTEXT.md` 时，grilling 中发现的新术语或修正**即时更新**，不攒到最后。
...
- **document-review（条件强制）** -- coherence / feasibility / product-lens / design-lens / security-lens / scope-guardian / adversarial 并行审查；Route C/Deep、高风险、或用户要求 review 时必须通过多轮门禁
...
| 信号 | 动作 |
|---|---|
| Route C / Deep brainstorm | 强制 `document-review <requirements-path>` |
| R-ID 数量 >8 | 强制 `document-review` |
| 外部上游/API/SDK、public endpoint、model alias、协议契约 | 强制 `document-review` |
| auth/token/凭证、私域素材、用户数据、计费/pricing/quota、审计日志 | 强制 `document-review` |
| 用户明确说 review / 审查 | 强制 `document-review` |
...
需求文档 (from Phase 1)
  |
  +-- 无 DESIGN.md --------> Route A: gstack-design-consultation -> gstack-design-shotgun
  |   (新项目无设计体系)        创建设计体系，再探索视觉方案
  |
  +-- 有 DESIGN.md, -------> Route B: gstack-design-shotgun
  |   无 approved.json        在现有体系内探索视觉方案
  |   (体系存在，无视觉方向)
  |
  +-- 有 DESIGN.md, -------> Route C: gstack-plan-design-review (推迟到 Phase 3)
...
| 场景 | 推荐组合 |
|---|---|
| 新项目，无设计体系 | `gstack-design-consultation` -> `gstack-design-shotgun` |
| 有体系，新功能需视觉方向 | `gstack-design-shotgun`（受 DESIGN.md 约束） |
| 有视觉方向，规划中补设计审查 | `gstack-plan-design-review`（在 Phase 3 中） |
| 实现中 | `frontend-design`（ce-work 自动启用） |
| 实现后视觉问题 | `gstack-design-review`（Phase 5 叠加） |
| 纯后端需求 | **跳过 Phase 2** |
...
- `gstack-design-shotgun` 用它约束变体生成
- `gstack-plan-design-review` 根据它校验设计一致性
- `gstack-design-html` 从中提取 token
- `gstack-design-review` 根据偏离程度判定严重性
...
ce-plan (唯一创建器)
    ...
    |  Implementation Units (每个 = 一个未来可提交的原子变更单元)
    |  Test Scenarios (枚举验证点，不预写代码)
    |
    v
document-review (Layer 1, 多轮门禁)
...
dev:plan 检测 Implementation Unit 数量，自动选审深度 (Layer 2)
    |
    +-- 10+ 单元 (大型) ------> gstack-autoplan (CEO + Design + Eng + DX, 串行；缺失则内置多角色 fallback)
    +-- 3-9 单元 (标准) ------> gstack-plan-eng-review (必需；缺失则 eng-review-protocol fallback) + gstack-plan-design-review (有 UI 时必需；缺失则 design-lens fallback)
    +-- 1-2 单元 (小型) ------> eng-review-protocol only
    +-- 涉及 API/CLI/SDK -----> + gstack-plan-devex-review (叠加；缺失则 feasibility/devex fallback)
    |
    v
...
| Layer 2 | 选中的 `gstack-plan-eng-review` / `gstack-autoplan` / `gstack-plan-design-review` / `gstack-plan-devex-review`，或对应内置 fallback，必须写入 REVIEW REPORT 并 cleared |
...
- Codex 没有阻塞提问工具时，用编号选项停下等待用户；不得把单轮主线程综合报告当作通过。
- gstack 审查技能不可用时，不跳过审查维度：执行对应内置 protocol/persona fallback；只有 fallback 也不可用或仍有 P0/P1 时才 STOP 请求用户裁决或记录 override。

### 反模式：水平切片

Implementation Unit **必须是端到端垂直切片**（一条薄路径穿透所有集成层：schema → API → UI → test），不是单层水平切片（"先写所有 model，再写所有 controller"）。

水平切片的危害：测试在想象中的行为上写，实现在想象中的接口上写，两者在集成时才发现不匹配。垂直切片让每个 Unit 独立可验证、可 demo。

```
错误（水平）:
  Unit 1: 所有 model
  Unit 2: 所有 controller
  Unit 3: 所有 view

正确（垂直）:
  Unit 1: 用户注册（model + route + form + test）
  Unit 2: 用户登录（model + route + form + test）
  Unit 3: 密码重置（model + route + email + test）
```
...
计划审查通过后，如果项目使用 Issue Tracker 且计划包含 3+ Implementation Units，`dev:plan` 自动建议运行 `to-issues` 将计划分解为独立可抓取的 Issue：
...
不强制——单人项目或 1-2 Unit 的小计划可跳过。

### 架构探索输入：`improve-codebase-architecture`

`ce-plan` 的研究子 Agent 在扫描代码库时，如果发现计划涉及的模块存在**浅模块**（接口几乎和实现一样复杂）、**缺失 seam**（无法在不编辑原处的情况下改变行为）、或**紧耦合泄漏**，自动引用 `improve-codebase-architecture` 的深化词汇（Module / Interface / Depth / Seam / Adapter / Leverage / Locality）描述问题，并在计划中标注"此 Unit 包含架构深化"。

不自动触发完整的 `/improve-codebase-architecture` 会话——那是独立活动。但计划中的架构语言必须精确。
...
> 路由器：`dev:code`
>
> 核心产出：代码 + staged/unstaged 工作树变更 + Implementation Unit 进度记录（不 commit）
...
每个 batch:
    ...
    -> ce-review mode:autofix (safe_auto only)
    |
    v
所有 Unit 完成:
    /simplify
    -> final ce-review mode:autofix
...
| 时机 | 动作 |
|---|---|
| 每个 2-3 Unit batch 完成后 | 运行 `ce-review mode:autofix plan:<path>` |
...
| 检测到的信号 | 自动触发技能 |
|---|---|
| 任何行为变更 | `test-driven-development` 默认强制：先写一个失败测试/失败断言（RED）→ 最小实现（GREEN）→ 重构；计划里的 `test-first` 只是强调测试策略，不是触发条件 |
| Implementation Unit 标记 `characterization-first` | 先写特征测试再修改 |
| Implementation Unit 标记 `external-delegate` | Codex 委托模式 (`ce-work-beta`) |
| 运行时遇到 bug | `diagnose`（构建 feedback loop → 复现 → 3-5 可证伪假设 → 仪器化 → 修复 + 回归测试 → 清理）；fallback: `ce-debug` |
...
所有 Unit 完成、准备进入 Phase 5 前，默认手动调用一次 `/simplify`，然后再运行 final `ce-review mode:autofix`。`/simplify` 并行派三个 agent（复用 / 质量 / 效率）扫整个累积 diff 并直接修复：
...
- ❌ Phase 5 的 `ce-review interactive` 完成后再回头跑（那是总体审查之后，不应回到实现期清理）
...
`/simplify` 会修改代码，所以其后必须再跑一次 final `ce-review mode:autofix`，确保 simplification edits 也经过 Phase 4 自动修复。

### 铁律

- **无失败测试，不写生产代码**
- **TDD 默认适用于所有行为变更**；例外仅限纯文档、机械配置、生成代码等，并需用户批准或计划记录
- **无根因分析，不尝试修复**
- **3 次修复失败，停下质疑架构**
...
dev:verify 自动叠加额外层:
    ...
    +-- 5+ UI 文件跨多页变更             -> gstack-qa (全站) + gstack-design-review | `dev-browser`（脚本化遍历多页路由）
...
- **Phase 5 PASS 必须覆盖全部命中层** -- core review、additive layers、todo-resolve 都无 blocker 后才算 PASS
...
| diff 修改 `claude-skills/`、`.agents/skills/`、`docs/ai-coding-workflow.md`、`scripts/supervise/`、`docs/supervise/` | 运行 `dev-supervise --scope project --since 7d`；若报告暴露 skill gap，开启 Phase 4/5 子流程并按 `writing-skills` RED-GREEN-REFACTOR 修复，不在 Phase 5.5 内直接 patch |
...
- 任一子步骤产生文件后，至少运行针对性验证（`git status --short`、链接/grep 检查、必要时 docs-only `/dev:verify`），并刷新绑定当前 HEAD/tree/diff 的 Phase 5 PASS 证据，再进入 Phase 6。
- 如果 `dev-supervise` 报告要求修改 `SKILL.md`，回到 Phase 4/5 完成修改和 `writing-skills` 验证后再重新进入 Phase 5.5；不得带着未处理的 P0/P1 流程问题进入 Phase 6。
...
验证通过且完成 Phase 5.5 交付前判断的代码
  ...
  |
  +-- [用户明确要求合并/push main] --------------------> Path A: Squash 合并
  |   squash merge + 中文 commit，保持主干线性干净 -> 自动删除当前 worktree
  |
  +-- [用户显式要求 PR] ------------------------------> Path B: PR 路径
  |   git-commit-push-pr -> gstack-land-and-deploy -> 自动删除当前 worktree
  |
...
| PR 有未解决的审查线程 | 先 `pr-comment-resolver` |
| Diff 涉及 UI 文件 | 并行 `feature-video` (PR 嵌入录屏) |
| 无部署配置文件 (greenfield) | 先 `setup-deploy` 配置部署 |
| 无绑定当前 HEAD/tree/diff 的 Phase 5 PASS 证据 | STOP，回 `/dev:verify` |

### Phase 6 确认收敛规则

- 用户显式进入 `/dev:ship`，或明确说"提交/commit/push/PR/上线/deploy"，只视为**进入交付流程**的授权；直接写 main 还需要用户明确说"合并到 main / push main / squash 到 main"。
- 不要把授权拆成 commit / push / merge / deploy 多次重复确认；那会把 Phase 6 退化成人工脚本播放器。
- 只有以下情况才继续自动执行：路径已被用户明确授权、Phase 5 PASS 证据绑定当前 diff、merge-readiness / deploy 报告无 blocker。
- `dev:flow mode:auto` 不构成 ship 授权；进入 Phase 6 前若无单独 ship 授权必须 STOP。
...
**为什么不把 Path A 作为隐式默认**：写 main 是破坏性/外部可见动作，必须由用户明确授权。个人项目或小团队仍可选 Path A；一旦用户明确要求合并到 main，squash 合并保持 main 线性，每个 commit 对应一个完整功能。

### Path B: PR 路径（仅在用户显式要求时）

```
git-commit-push-pr (ce-pr-description 生成描述 -> 自适应 PR)
  -> gstack-land-and-deploy (CI -> merge-readiness 报告 -> 合并 -> 部署 -> 金丝雀)
  -> 自动删除当前 task worktree + 本地 feature branch
```
...
```
gstack-ship (合并基准分支 -> 测试 -> 覆盖率审计 60%/80% -> 计划完成度审计
      -> pre-landing review 多轮 -> adversarial review 多轮
      -> VERSION bump -> CHANGELOG 生成 -> 可二分提交)
  -> gstack-document-release (交叉引用 diff 同步所有文档)
  -> gstack-land-and-deploy (同上)
  -> 自动删除当前 task worktree + 本地 feature branch
```
...
| 受影响路由的回归用例（Phase 5 自动叠加） | `test-browser` | 与 ce-review 联动产生结构化报告 |
...
已验证或已完成的工作
  ...
  |
  +-- [方向被拒绝] ("不做", "out of scope",  -> Route A2: .out-of-scope/
  |   "wontfix", 用户明确否决某方向)           写入 .out-of-scope/<slug>.md
  |                                            记录：被拒方向 + 为什么不做 + 先前请求
  |                                            避免同一方向被反复评估
  |
  +-- [架构摩擦暴露] (diagnose/修复后标记     -> Route A3: improve-codebase-architecture
  |   "无好的测试 seam"、"浅模块"、             grilling loop: 探索 -> 候选列表 -> 用户选 -> 深化设计
  |   "紧耦合泄漏"、Phase 4 标记)              产出: CONTEXT.md 更新 + 可选 ADR + 深化 Issue
  |                                            用 Module/Interface/Depth/Seam/Leverage 词汇
  |
...
分析 gap（哪条路由/规则需要改）
    ...
    |
    +-- [重复发现] ------> 1. 先跑 old skill / old routing baseline，记录失败或误判
    |                       2. 从 learnings.jsonl 聚合模式，定位缺失的路由规则或检测信号
    |                       3. 用 writing-skills 的 RED/GREEN/REFACTOR 闭环更新技能
    |
    +-- [流程摩擦] ------> 1. 先跑能复现该摩擦的压力场景
    |                       2. 从 retro 提取摩擦点并追溯到具体 Phase 的 Scene Detection
    |                       3. 仅做能消除该失败模式的最小修订，再复测
    |
    ...
    |
    v
验证: 改进后的技能在同一历史场景/压力场景中路由正确，且没有新误判
    |
    v
...
- **自动推进模式**: `mode:auto`，阶段通过且无阻塞时自动继续非破坏性步骤；不授权 `/dev:ship`
...
| 想一路自动推进非破坏性阶段，并在交付前停下要授权 | `dev:flow mode:auto` |
| 有明确计划，一键执行 | `lfg` |
...
| 2 | **测试先于代码** -- 无失败测试不写生产代码 | tdd, test-driven-development |
...
| 6 | **工作区先于工作** -- 创建任何文档或代码前必须在任务专属 worktree / feature branch 中，否则先创建工作区 | dev:flow Signal 0, using-git-worktrees |
| 7 | **提交由用户触发** -- AI 在 Phase 1-5 只修改文件、运行测试，不主动 `git commit` / `git push` / 创建 PR；提交仅在用户显式调用 `/dev:ship` 或说"提交/commit/push/PR"时进行 | dev:ship, git-commit-push-pr, gstack-ship |
...
Phase 1 特例：Route C/Deep 或高风险需求的 `document-review` 是进入 Phase 3 前的强制门禁；若仍有未处理 P0/P1，只能阻塞或记录用户 override，不能直接宣布 requirements 批准。

Phase 3 特例：`document-review` 和选中的计划审查层是进入 Phase 4 前的强制门禁；若仍有未处理 P0/P1，只能阻塞或记录用户 override，不能直接宣布计划已审查通过。
...
需求文档            DESIGN.md          实施计划            ce-work         ce-review       pre-ship learn   交付
R1,R2,R3 -------> Token/色值 -------> Req Trace -------> 实现代码 -----> R-ID 验证 ----> docs/solutions -> PR
                                       Impl Units         工作树变更       safe_auto       docs/supervise    部署
...
| 来源 | 角色 | 安装后效果 |
|---|---|---|
| **gstack** | 专业工具 | 设计探索（gstack-design-shotgun）、全站 QA、安全审计（CSO）、性能基线、部署金丝雀 |
